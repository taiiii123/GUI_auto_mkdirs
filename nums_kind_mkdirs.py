...
# 数字表記のディレクトリ名を作成する関数
def mkdir_nums(dirPath , dir_name, dir_count):
    dir_count = int(dir_count)
    if dir_count < 10:
        for count in range(1, dir_count + 1):
            os.mkdir(dirPath + '/' + '{}_0{}'.format(dir_name, count))
    else:
        for count in range(1, 10):
            os.mkdir(dirPath + '/' + '{}_0{}'.format(dir_name, count))
        for count in range(10, dir_count + 1):
            os.mkdir(dirPath + '/' + '{}_{}'.format(dir_name, count))

# 数字(No.付き)表記のディレクトリ名を作成する関数
def mkdir_numbers(dirPath, dir_name, dir_count):
    for count in range(1, int(dir_count) + 1):
        os.mkdir(dirPath + '/' + '{}_No.{}'.format(dir_name, count))


# 英語表記(One~)のディレクトリ名を作成する関数
...
def mkdir_kansuuji(dirPath, dir_name, dir_count):
    for count in range(0, int(dir_count)):
        os.mkdir(dirPath + '/' + '{}_{}'.format(dir_name, KANSUUJI[count]))

# ローマ数字表記(Ⅰ~)のディレクトリ名を作成する関数
def mkdir_roman_numbers(dirPath, dir_name, dir_count):
    for count in range(0, int(dir_count)):
        os.mkdir(dirPath + '/' + '{}_{}'.format(dir_name, ROMAN_NUMBERS[count]))